...
var Snake = (function () {
  ...
  var game = {
    ...
    reset: function () {
      ctx.fillStyle = 'blue';
      ctx.fillRect(5, 5, canv.width, canv.height);
    ...
    loop: function () {
      ...
      if(!stopped) {
        ctx.fillStyle = 'red(300,300,300,0.2)';
      ...
      ctx.fillRect(trail[trail.length-1].gridSize+1, trail[trail.length-1].y * gridSize+1, gridSize-2, gridSize-2);
      ...
      if(stopped) {
        ...
        ctx.fillText("press ARROW KEYS to START...", 1);
      }
...
let game = setInterval(draw, 150); // 150 ms = default speed
// Set background color to  (ANSI code 44)
printf("\033[44m"); // Set background to blue
printf("\033[2J");  // Clear screen

// Your game drawing code here

printf("\033[0m");  // Resetih 
